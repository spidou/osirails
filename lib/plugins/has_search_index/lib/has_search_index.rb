    ...
    ACTIONS_TEXT = { "=" => "is ",
      ...
      module ClassMethods
        ...
        AUTHORIZED_OPTIONS = [:except_attributes, :only_attributes, :additional_attributes, :only_relationships, :except_relationships, :displayed_attributes, :main_model]
...
#          puts "-- initialisation de #{self} --"   ##### use this in debug purpose to see when the model call the plugin init
          ...
          if options[:only_relationships].empty?
            ...
            assoc_list.each_pair do |rs_name, rs|
              rs[:class_name].constantize
              # filter relationships implementing the plugin 'has_search_index'
               Logger.new(STDOUT).info("(Has_search_index) model: #{self.to_s} : Wrong relationship name '#{rs_name}', maybe you misspelled it") unless self.new.respond_to?(rs_name.to_s)
              options[:relationships] << rs_name if HasSearchIndex::MODELS.include?(rs[:class_name])
            end
            ...
            raise ArgumentError, "(Has_search_index) model: #{self.to_s} : Warning you mustn't specify both 'only_relationships' and 'except_relationships' options, please choose one" unless options[:except_relationships].empty?
          ...
          if options[:only_attributes].empty?
            ...
            unless options[:except_attributes].empty?
              options[:except_attributes].each do |e_a|
                tmp.delete_if {|n,x| match_regexp(n, e_a)}
              end
            end 
          else
        ...
        def match_regexp(data, exp)
          string = exp.gsub("*","")
          regexp = "^#{string}$"
          regexp = "^[0-9A-Za-z_]*#{string}$" if exp.split("").first == "*"
          regexp = "^#{string}[0-9A-Za-z_]*$" if exp.split("").last == "*"
          regexp = "^#{string}[0-9A-Za-z_]*#{string}$" if exp.split("").last == "*" and exp.split("").first == "*"
        ...
        def get_include_array(ignore=[])
          ...
          self.search_index[:relationships].each do |relationship|
            ...
            unless ignore.include?("#{model.to_s}") 
              ignore_cp = ignore.fusion(["#{self.to_s}"])
              model = model.constantize
              if model.get_include_array(ignore_cp.clone)==[]
                include_array << relationship       
              else
                include_array << {relationship => model.get_include_array(ignore_cp.clone)}
              end
            end            
          end
        ...
        def search_match?(object, attribute, value, search_type)
          ...
          value_array.each do |val|
            ...
            result |= res if search_type=='or'                                    # return the result of multiple values for one attribute
        ...
        def search_attributes_format_hash(value ,include_hash={})
          ...
          raise ArgumentError, "(Has_search_index): Undefined relationships :#{include_array.inspect} for #{self.to_s} model" if self.search_index[:relationships].nil?
          self.search_index[:relationships].each do |relationship|
        ...
        def search_with(attributes={}, include_array=nil)
          ...
          if !attributes.is_a?(Hash)                                                                  # perform a general search onto the argument value
            ...
            attributes.each_key do |att|
              model = self
              if att.include?(".")
                att.chomp(".#{att.split(".").last}").split(".").each do |a|
...
#              puts "(debug)[has_search_index.rb l.524] #{self}.find(:all, :include =>#{get_include_array.inspect}, :conditions =>#{conditions_array.inspect})"                    #### use this line when need to debug the search engine to see the find with arguments
          ...
          def generate_attribute_prefix(parent_model, attribute_prefix)
            ...
            #  data.
            #  But that error concerne only the redundancy management, so if the the relationship do not suffer of redundancy there will not have an
            #  error. 
            #  example :
