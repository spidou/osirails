class ClosedPurchaseOrdersController < ApplicationController
  ...
  def index
    ...
    if params['filter'] == PurchaseOrder::STATUS_CANCELLED
      @closed_purchase_orders = PurchaseOrder.all(:conditions => ["status = ? or status = ?", PurchaseOrder::STATUS_COMPLETED, PurchaseOrder::STATUS_CANCELLED]).paginate(:page => params[:page], :per_page => ClosedPurchaseOrder::REQUESTS_PER_PAGE)
    end
  end
