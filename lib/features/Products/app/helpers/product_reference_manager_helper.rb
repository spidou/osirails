module ProductReferenceManagerHelper
  ...
  def get_children(categories,list)
    categories.each do |category|
      ...
      list << "<li><span class='category'>#{category.name} &nbsp; (#{category.product_references_count})</span>&nbsp; <span class='action'>"+
      ...
      if category.product_references.size > 0
        category.product_references.each do |reference|
          list << "<li><span class='reference'>#{reference.name} &nbsp; (#{reference.products_count})</span>&nbsp; <span class='action'>"+
