    ...
    when 'enable'
      if @feature.enable
        ...
        unless @feature.activate_dependencies.empty?
          @feature.activate_dependencies.many? ? flash[:error] << "Les modules suivants sont requis : " : flash[:error] << "Le module suivant est requis : "
          deps = []
    ...
    when 'disable'
      if @feature.disable
        ...
        unless @feature.deactivate_children.empty?
          @feature.deactivate_children.many? ? flash[:error] << "D'autres modules dépendent de ce module : " : flash[:error]  << "Un autre module dépend de ce module : "
          deps = []
    ...
    when 'install'
      if @feature.install
        ...
        unless @feature.able_to_install_dependencies.empty?
          @feature.able_to_install_dependencies.many? ? flash[:error] << "Les modules suivants sont requis : " : flash[:error] <<  "Le module suivants est requis : "
          deps = []
        ...
        unless @feature.able_to_install_conflicts.empty?
          @feature.able_to_install_conflicts.many? ? flash[:error] << "<br/>Les conflits suivants ont été détectés : " : flash[:error] << "<br />Le conflit suivant a été détecté : "
          deps = []
