module RolesHelper
  ...
  def show_edit_button(role)
    if controller.can_edit?(current_user)
      link_to(image_tag("/images/edit_16x16.png", :alt =>"Modifier", :title =>"Modifier"), edit_role_path(role))
    end
  end
  
  # This method permit to test permission for view button
  def show_view_button(role)
    if controller.can_view?(current_user)
      link_to(image_tag("/images/view_16x16.png", :alt =>"D&eacute;tails", :title =>"D&eacute;tails"), role_path(role)) 
    end
  end
  
  # This method permit to test permission for delete button
  def show_delete_button(role)
    if controller.can_delete?(current_user)
      link_to(image_tag("/images/delete_16x16.png", :alt => "Supprimer", :title => 'Supprimer'), role, {:method => :delete, :confirm => 'Etes vous s&ucirc;r ?'})
    end
  end
  ...
  def show_add_button
    if controller.can_add?(current_user)
      ...
      add_button << link_to("<img src='/images/add_16x16.png' alt='Ajouter' title='Ajouter' /> Ajouter un role", new_role_path)
