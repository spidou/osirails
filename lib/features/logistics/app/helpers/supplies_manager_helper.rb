module SuppliesManagerHelper
  ...
  def reactivate_category_link(supply_category)
    if supply_category.class.can_reactivate?(current_user)
      link_to(image_tag("tick_16x16.png", :alt => "Résactiver", :title => "Réactiver"), self.send("reactivate_#{supply_category.class.name.underscore}_path",supply_category)) if supply_category.can_be_reactivated?
    end
  end
  ...
  def edit_supply_link(supply, button = false)
    if supply.class.can_edit?(current_user)
      ...
      link_to(image_tag("edit_16x16.png", :alt => "Éditer", :title => "Éditer") + text, send("edit_#{supply.class.name.tableize.singularize}_path",supply)) unless !supply.enable
    end
  end
  ...
  def reactivate_supply_link(supply, button = false)
    if supply.class.can_reactivate?(current_user)
      ...
      link_to(image_tag("tick_16x16.png", :alt => "Résactiver", :title => "Réactiver") + text, self.send("reactivate_#{supply.class.name.underscore}_path",supply)) if supply.can_be_reactivated?
    end
  end
