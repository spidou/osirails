  ...
  def show_closed_button(inventory)
    if Inventory.can_edit?(current_user)
      if inventory.inventory_closed?
        image_tag("/images/lock_16x16.png", :alt => "Clôturé") + " Clôturé"
      else
        link_to("Clôturer l'inventaire", {:id => inventory.id}, {:method => :put, :class => 'link_to_closed', :confirm => "Attention, une fois clôturé, vous ne pourrez plus modifier l'inventaire"})
      end
    end
  end
...
#  end
  ...
  def in_place_editor(inventory,commodity)
    if Inventory.can_edit?(current_user)
      return inventory.inventory_closed? ? "<span>#{commodity.quantity}</span>" : editable_content_tag(:span, commodity, "quantity", true, nil, {:class => 'in_line_editor_span'}, {:clickToEditText => 'Cliquer pour modifier...', :savingText => 'Mise à jour', :submitOnBlur => true, :cancelControl => false, :okControl => false})
    end
  ...
  def show_structured_commodities(inventory)
    ...
    parent_group.each do |parent|
      ...
      structured_commodities << "<td><img id='commodity_category_#{commodity_category.parent_commodity_category_id}_develop' src='/images/add_10x10.png' alt='Dérouler' title='Dérouler' onclick='develop(this.ancestors()[1])' style='display: none;'/> "
      ...
      structured_commodities << "<td class='commodity_category'><span class='commodity_category_#{commodity_category.parent_commodity_category_id}_total'>#{show_total(inventory,commodity_category,1)}</span> €</td>"
      ...
      parent.last.group_by(&:commodity_category_id).sort.each do |group|
        ...
        structured_commodities << "<td><img id='commodity_category_#{sub_commodity_category.commodity_category_id}_develop' src='/images/add_10x10.png' alt='Dérouler' title='Dérouler' onclick='develop(this.ancestors()[1])' style='display: none;' /> "
        ...
        structured_commodities << "<td class='sub_commodity_category'><span class='sub_commodity_category_#{sub_commodity_category.commodity_category_id}_total' >#{show_total(inventory,sub_commodity_category)}</span> €</td>"
        structured_commodities << "</tr>"
        group.last.each do |commodity|
          ...
          structured_commodities << "<td>#{commodity.fob_unit_price} €/#{unit_measure.symbol}</td>"
          structured_commodities << "<td>#{commodity.taxe_coefficient} %</td>"
          structured_commodities << "<td><span id='commodities_inventory_#{commodity.id}_price'>#{commodity.fob_unit_price + ((commodity.fob_unit_price * commodity.taxe_coefficient)/100)}</span> €/#{unit_measure.symbol}</td>"
