...
class ToolTest < ActiveSupport::TestCase
  ...
  def test_status_with_incident_today
    [ToolEvent::UNAVAILABLE, ToolEvent::AVAILABLE].each do |status|
      add_event(@good_tool, Date.today, ToolEvent::INCIDENT, status)
      assert_equal status, @good_tool.status, "tool status should be equal to last event's status"
    end

    add_event(@good_tool, Date.today, ToolEvent::INCIDENT, ToolEvent::SCRAPPED)
  ...
  def test_status_with_incident_yesterday
    [ToolEvent::UNAVAILABLE, ToolEvent::AVAILABLE].each do |status|
      add_event(@good_tool, Date.yesterday, ToolEvent::INCIDENT, status)
      assert_equal ToolEvent::AVAILABLE, @good_tool.status, "tool status should be Available because the event is finished"
    end

    add_event(@good_tool, Date.yesterday, ToolEvent::INCIDENT, ToolEvent::SCRAPPED)
    assert_equal ToolEvent::SCRAPPED, @good_tool.status, "tool status should be Scrapped because the scrapped status is definitive even if the event is finished"
  end
  
  def test_status_with_last_event_finishing_today
    [ToolEvent::UNAVAILABLE, ToolEvent::AVAILABLE].each do |status|
      add_event(@good_tool, Date.today, ToolEvent::INTERVENTION, status)
      assert_equal status, @good_tool.status, "tool status should be equal to last event's status"
    end
  end
  ...
  def test_status_with_last_event_finishing_yesterday   
    [ToolEvent::UNAVAILABLE, ToolEvent::AVAILABLE].each do |status|
      add_event(@good_tool, Date.yesterday, ToolEvent::INTERVENTION, status)
      assert_equal ToolEvent::AVAILABLE, @good_tool.status, "tool status should be Available because the event is finished"
    end
  end
  ...
  def test_has_one_scrapped
    assert !@good_tool.scrapped?, "scrapped should be false because good_tool has no event with SCRAPPED status"
    
    add_event(@good_tool, Date.today, ToolEvent::INCIDENT, ToolEvent::SCRAPPED)
    assert @good_tool.scrapped?, "scrapped? should be true because good_tool has an event with SCRAPPED status"
  end
  
  private
  
    def add_event(tool, date, event_type, status)
      event = ToolEvent.new(:event_type => event_type, :start_date => date, :end_date => date, :tool_id => tool.id, :status => status, :comment => "comment")
      flunk "event should be valid #{event.errors.inspect}" unless event.save
      event
    end
