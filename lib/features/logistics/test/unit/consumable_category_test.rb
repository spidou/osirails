...
class ConsumableCategoryTest < ActiveSupport::TestCase
  ...
  def test_has_many_enabled_and_disabled_supplies
    ...
    flunk "stock flow must be saved" unless new_stock_flow(@pvc,@pvc.suppliers.first,true,5)
    sleep(1)
    flunk "stock flow must be saved" unless new_stock_flow(@pvc,@pvc.suppliers.first,false,5)
    flunk "@pvc should be disabled" unless @pvc.disable
