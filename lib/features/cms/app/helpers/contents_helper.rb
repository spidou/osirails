  ...
  def show_contents_versions(content_versions)
    if Content.can_view?(current_user)
      ...
      content_versions.each_with_index do |content_version, index|
        ...
        contents_versions_list << link_to(content_version.versioned_at.humanize, :controller => :content_versions, :action => :show, :content_id => content_version.content.id, :version => index+1 )
        contents_versions_list << "</li>"             
      end
