module FeaturesHelper
  ...
  def display_install_link(feature)
    if feature.base_feature? and feature.name == "Admin" 
        button= "<a class=\"admin_features_activated-installed\" href=\" \" title='Vous ne devez pas désinstaller le module "+feature.name+" car il appartient au noyau  !' Onclick=\"alert ('Vous ne devez pas désinstaller le module "+feature.name+" car il appartient au noyau  ! ');return false\" >Oui</a>" if feature.activated?
      ...
      # button = link_to("<div class=\"admin_features_activated-installed\">Oui</div>", {:action => "index", :id => feature.id}, :title=>'Vous devez désactiver le module avant de le désactiver!', :confirm => 'Vous devez désactiver le module'+feature.name, :method => :get)
       button = "<a class=\"admin_features_activated-installed\" href=\" \" title='Vous devez désactiver le module "+feature.name+"!' OnClick=\"alert ('Vous devez désactiver le module "+feature.name+"'); return false\">Oui</a>"
    else
      if feature.installed?
        button = link_to("<div class=\"admin_features_activated-installed\" >Oui</div>", {:action => "show", :id => feature.id}, :title=>'Cliquez ici pour désinstaller', :method => :get)
      else 
        button = link_to("<div class=\"admin_features_non_activated-installed\"  >Non</div>", {:action => "install", :id => feature.id}, :title=>'Cliquez ici pour installer', :confirm => 'Voulez-vous installer le module ['+feature.name+'] ?', :method => :put) 
      end 
      button
    end 
  end    
  ...
  def display_activate_link(feature)
    if !feature.installed?
      ...
      button = "<a class=\"admin_features_non_activated-installed\" href=\" \" title='Vous devez installer le module "+feature.name+"!' OnClick=\"alert ('Vous devez installer le module "+feature.name+"'); return false\">Non</a>"
    elsif feature.base_feature? and feature.name == "Admin" 
      if feature.activated?
        button= "<a class=\"admin_features_activated-installed\" href=\" \" title='Vous ne devez pas désactiverr le module "+feature.name+"car il appartient au noyau!' Onclick=\"alert ('Vous ne devez pas désactiverr le module "+feature.name+"car il appartient au noyau!');return false\" >Oui</a>"
      end
    else
     if feature.activated? 
        button = link_to("<div class=\"admin_features_activated-installed\">Oui</div>", {:action => "disable", :id => feature.id},  :title=>'Cliquez ici pour désactiver', :confirm => 'Voulez-vous desactiver le module ['+feature.name+'] ?', :method => :put) 
      else
        button = link_to("<div class=\"admin_features_non_activated-installed\">Non</div>", {:action => "enable", :id => feature.id},  :title=>'Cliquez ici pour activer', :confirm => 'Voulez-vous activer le module ['+feature.name+'] ?', :method => :put)
      end
    end
     button 
  end
