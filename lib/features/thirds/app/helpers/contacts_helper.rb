module ContactsHelper
  ...
  def display_contacts_and_add_contact_button(owner, contacts, params, new_contact_number, error)
    ...
    html = "<h2>Contacts</h2>"
    if contact_controller.can_list?(current_user) and Establishment.can_list?(current_user)
      html += "<p>"
      html += display_contacts(owner, contacts)
      html += "</p>"
    end
    
    if contact_controller.can_add?(current_user) and Establishment.can_add?(current_user)
      html += "<p>"
  ...
  def display_add_contact_button(owner, params, new_contact_number, error)
    render :partial => 'contacts/new_contact', :locals => {:owner_type => owner.class.to_s,
      :owner_id => owner.id, :params => params, :new_contact_number => new_contact_number,
      :error => error}
  end
