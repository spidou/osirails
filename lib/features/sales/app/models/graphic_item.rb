class GraphicItem < ActiveRecord::Base
  ...
  def add_to_graphic_item_spool(type,user)
    ...
    spools_directory = "#{RAILS_ROOT}/#{GraphicItemSpoolItem::SPOOLS_DIRECTORY}"
    
    unless File.exists?("#{spools_directory}/#{gisi.user_spool_directory}/")
      system "mkdir -p #{spools_directory}/#{gisi.user_spool_directory}/"
    end
