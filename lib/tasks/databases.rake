namespace :db do
  ...
  override_task :migrate => :environment do
    ...
    FeatureManager.ordered_feature_paths.select{|path| File.directory?("#{path}/db/migrate")}.each do |feature_path|  # this code has been added to take in account
  ...
  namespace :migrate do
    ...
    override_task :up => :environment do
      ...
      FeatureManager.ordered_feature_paths.select{|path| File.directory?("#{path}/db/migrate")}.each do |feature_path|  # the features' migrations
  ...
  override_task :rollback => :environment do
    ...
    FeatureManager.ordered_feature_paths.select{|path| File.directory?("#{path}/db/migrate")}.reverse.each do |feature_path|  # this code has been added to take in account
      puts "====== Running migrations inside #{feature_path} ======"                                                          # the features' migrations
