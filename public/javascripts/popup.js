...
Popup.prototype = {
  ...
  */
  initialize: function(popup, link) {
    ...
    options.show_delay = this.first_value(options.show_delay, Popup.show_delay, 1500);
    options.hide_delay = this.first_value(options.hide_delay, Popup.hide_delay, 20);
  ...
  */
  ...
  show: function(event) {
    ...
    Element.setStyle(this.popup, {position: 'absolute', top: pos.y, left: pos.x, zIndex: Popup.zIndex++ });
    this.is_open = true;
  ...
  */
  ...
  get_center_position: function() {
    ...
    var viewport_height = document.height - 100;
...
function update_acts_as_watchable_buttons (id){
    ...
    $("span_" + id).update($("acts_as_watchables_button").innerHTML)
    $("acts_as_watchables_button").update('')
}
